...
function ext() {
  const ext = Deno.readTextFileSync("browser/Extensions.bak");
...
async function bm() {
  const json = await Deno.readTextFile("./browser/Bookmarks.bak")
    .then((res) => JSON.parse(res));

  const obj = json.roots.bookmark_bar.children;
  const bookmarks = obj.filter((val) => val.id === "701");
...
const title = `AK#${repoInfo.name.charAt(0).toUpperCase() + repoInfo.name.slice(1)}`;
